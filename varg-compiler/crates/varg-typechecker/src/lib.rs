...
impl TypeError {
    ...
    pub fn message(&self) -> String {
        match self {
            ...
            TypeError::AgentSpawnUnknown { spawner, target } => {
                format!("agent `{}` spawns `{}`, but `{}` is not defined in this program", spawner, target, target)
            }
...
impl TypeChecker {
    ...
    fn unknown_method_error(&self, type_name: &str, method_name: &str) -> TypeError {
        ...
        let builtins = [
            ...
            // Wave 36: Tensor (ndarray)
            "tensor_zeros", "tensor_ones", "tensor_eye", "tensor_from_list",
            "tensor_shape", "tensor_reshape", "tensor_slice",
            "tensor_add", "tensor_sub", "tensor_mul_scalar", "tensor_matmul", "tensor_dot",
            "tensor_sum", "tensor_mean", "tensor_max", "tensor_min", "tensor_to_list",
            // Wave 38: DataFrame (Polars)
            "df_read_csv", "df_read_parquet", "df_write_csv", "df_write_parquet",
            "df_select", "df_filter", "df_sort", "df_groupby", "df_agg",
            "df_head", "df_with_column", "df_shape", "df_columns",
            // Wave 40: Local Embeddings + DuckDB
            "embed_local", "embed_local_batch",
            "duckdb_open", "duckdb_query", "duckdb_execute", "duckdb_close",
            // Wave 41: Full-Text Search + Hybrid RAG
    ...
    /// Wave 39: Build spawn adjacency map, validate all targets exist, detect cycles via DFS.
    fn check_agent_graph(&self, program: &Program) -> Vec<TypeError> {
        use std::collections::{HashMap, HashSet};

        // Collect all defined agent names
        let agent_names: HashSet<String> = program.items.iter()
            .filter_map(|i| if let Item::Agent(a) = i { Some(a.name.clone()) } else { None })
            .collect();

        // Build spawn edges: agent_name → [spawned_agent_names]
            ...
            if let Item::Agent(agent) = item {
                ...
                for method in &agent.methods {
                    if let Some(body) = &method.body {
                        self.collect_spawn_targets_in_block(body, &mut targets);
                    }
                }
                for target in &targets {
                    if !agent_names.contains(target) {
                        type_errors.push(TypeError::AgentSpawnUnknown {
                            spawner: agent.name.clone(),
                            target: target.clone(),
                        });
                    }
                }
                edges.insert(agent.name.clone(), targets);
            }
        }
        ...
        for name in &agent_names {
            if !visited.contains(name) {
                if let Some(cycle) = self.dfs_cycle(name, &edges, &mut visited, &mut rec_stack) {
                    type_errors.push(TypeError::AgentGraphCycle { cycle });
                }
            }
        }
    ...
    fn collect_spawn_targets_in_block(&self, block: &Block, out: &mut Vec<String>) {
        for stmt in &block.statements {
            self.collect_spawn_targets_in_stmt(stmt, out);
        }
    }

    fn collect_spawn_targets_in_stmt(&self, stmt: &Statement, out: &mut Vec<String>) {
        match stmt {
            Statement::Expr(e) | Statement::Let { value: e, .. } | Statement::Return(Some(e)) => {
                self.collect_spawn_targets_in_expr(e, out);
            }
            Statement::If { condition, then_block, else_block } => {
                self.collect_spawn_targets_in_expr(condition, out);
                self.collect_spawn_targets_in_block(then_block, out);
                if let Some(eb) = else_block { self.collect_spawn_targets_in_block(eb, out); }
            }
            Statement::While { condition, body } => {
                self.collect_spawn_targets_in_expr(condition, out);
                self.collect_spawn_targets_in_block(body, out);
            }
            Statement::Foreach { collection, body, .. } => {
                self.collect_spawn_targets_in_expr(collection, out);
                self.collect_spawn_targets_in_block(body, out);
            }
            Statement::TryCatch { try_block, catch_block, .. } => {
                self.collect_spawn_targets_in_block(try_block, out);
                self.collect_spawn_targets_in_block(catch_block, out);
            }
            Statement::UnsafeBlock(b) => self.collect_spawn_targets_in_block(b, out),
            _ => {}
        }
    }

    fn collect_spawn_targets_in_expr(&self, expr: &Expression, out: &mut Vec<String>) {
        match expr {
            Expression::Spawn { agent_name, .. } => { out.push(agent_name.clone()); }
            Expression::MethodCall { caller, args, .. } => {
                self.collect_spawn_targets_in_expr(caller, out);
                for a in args { self.collect_spawn_targets_in_expr(a, out); }
            }
            Expression::BinaryOp { left, right, .. } => {
                self.collect_spawn_targets_in_expr(left, out);
                self.collect_spawn_targets_in_expr(right, out);
            }
            Expression::Await(e) | Expression::TryPropagate(e) | Expression::UnaryOp { operand: e, .. } => {
                self.collect_spawn_targets_in_expr(e, out);
            }
            Expression::ArrayLiteral(elems) | Expression::TupleLiteral(elems) => {
                for e in elems { self.collect_spawn_targets_in_expr(e, out); }
            }
            _ => {}
        }
    }
        ...
        if let Some(neighbors) = edges.get(node) {
            for neighbor in neighbors {
                if !visited.contains(neighbor) {
                    if let Some(cycle) = self.dfs_cycle(neighbor, edges, visited, rec_stack) {
                        return Some(cycle);
                    }
                } else if rec_stack.contains(neighbor) {
                    // Found a cycle — extract the cycle path
                    let start = rec_stack.iter().position(|n| n == neighbor).unwrap();
    ...
    fn infer_expression_type(&mut self, expr: &Expression) -> Result<TypeNode, TypeError> {
        match expr {
            ...
            Expression::MethodCall { caller, method_name, args } => {
                ...
                // ===== Wave 36: ndarray Tensor Builtins =====
                } else if method_name == "tensor_zeros" || method_name == "tensor_ones" {
                    if args.len() != 1 { return Err(TypeError::TypeMismatch { expected: "1 argument (shape)".to_string(), found: format!("{} arguments", args.len()) }); }
                    Ok(TypeNode::Custom("TensorHandle".to_string()))
                } else if method_name == "tensor_eye" {
                    if args.len() != 1 { return Err(TypeError::TypeMismatch { expected: "1 argument (n)".to_string(), found: format!("{} arguments", args.len()) }); }
                    Ok(TypeNode::Custom("TensorHandle".to_string()))
                    ...
                    if args.len() != 2 { return Err(TypeError::TypeMismatch { expected: "2 arguments (a, b)".to_string(), found: format!("{} arguments", args.len()) }); }
                    Ok(TypeNode::Float)
                } else if method_name == "tensor_sum" || method_name == "tensor_mean"
                       || method_name == "tensor_max" || method_name == "tensor_min" {
                    if args.len() != 1 { return Err(TypeError::TypeMismatch { expected: "1 argument (tensor)".to_string(), found: format!("{} arguments", args.len()) }); }
                    Ok(TypeNode::Float)
                ...
                // ===== Wave 38: Polars DataFrame Builtins =====
                    ...
                    if args.len() != 1 { return Err(TypeError::TypeMismatch { expected: "1 argument (df)".to_string(), found: format!("{} arguments", args.len()) }); }
                    Ok(TypeNode::Array(Box::new(TypeNode::String)))
                // ===== Wave 40: Local Embeddings (no OCAP needed) =====
                    ...
                    if args.len() != 1 { return Err(TypeError::TypeMismatch { expected: "1 argument (texts)".to_string(), found: format!("{} arguments", args.len()) }); }
                    Ok(TypeNode::Array(Box::new(TypeNode::Array(Box::new(TypeNode::Float)))))
                // ===== Wave 40: DuckDB (requires DbAccess) =====
                    ...
                    Ok(TypeNode::Void)
                // ===== Wave 41: Full-Text Search (requires FileAccess) =====
            ...
            // Wave 37: llm_structured<T>(provider, model, prompt) → Custom(T)
            Expression::GenericCall { func_name, type_arg, args } => {
                match func_name.as_str() {
                    "llm_structured" => {
                        if args.len() != 3 {
                            return Err(TypeError::TypeMismatch {
                                expected: "3 arguments (provider, model, prompt)".to_string(),
...
mod tests {
    ...
    fn test_spawn_returns_agent_handle() {
        let mut checker = TypeChecker::new();
        let worker_agent = Item::Agent(AgentDef {
        ...
        let program = Program {
            no_std: false, docs: std::collections::HashMap::new(),
            items: vec![worker_agent, Item::Agent(AgentDef {
                name: "Test".to_string(),
                is_system: false, is_public: false,
                target_annotation: None, annotations: vec![],
                    implements: vec![],
    ...
    // ===== Wave 39: Agent Graph Validation Tests =====

    fn make_agent(name: &str, spawns: Vec<&str>) -> Item {
        let stmts: Vec<Statement> = spawns.iter().map(|target| {
            Statement::Expr(Expression::Spawn { agent_name: target.to_string(), args: vec![] })
        }).collect();
        Item::Agent(AgentDef {
            name: name.to_string(),
            is_system: false, is_public: false,
    ...
    #[test]
    fn test_wave39_valid_spawn_graph() {
        let mut checker = TypeChecker::new();
        let program = Program {
            no_std: false, docs: Default::default(),
            items: vec![
                make_agent("Worker", vec![]),
                make_agent("Supervisor", vec!["Worker"]),
            ],
        };
        assert!(checker.check_program(&program).is_ok(), "linear spawn graph should be valid");
    }

    #[test]
    fn test_wave39_spawn_unknown_agent_errors() {
        let mut checker = TypeChecker::new();
        let program = Program {
            no_std: false, docs: Default::default(),
            items: vec![make_agent("Boss", vec!["Ghost"])],
        };
        let errs = checker.check_program(&program).unwrap_err();
        assert!(errs.iter().any(|e| matches!(&e.error, TypeError::AgentSpawnUnknown { target, .. } if target == "Ghost")),
            "should flag spawn of undefined agent");
    }

    #[test]
    fn test_wave39_cycle_detected() {
        let mut checker = TypeChecker::new();
        let program = Program {
            no_std: false, docs: Default::default(),
            items: vec![
    ...
    #[test]
    fn test_wave39_self_cycle_detected() {
        let mut checker = TypeChecker::new();
        let program = Program {
            no_std: false, docs: Default::default(),
            items: vec![make_agent("Looper", vec!["Looper"])],
        };
        let errs = checker.check_program(&program).unwrap_err();
        assert!(errs.iter().any(|e| matches!(&e.error, TypeError::AgentGraphCycle { .. })),
