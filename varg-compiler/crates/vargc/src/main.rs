...
/// `vargc doctor` — print a system diagnostics table
fn cmd_doctor() {
    println!("vargc doctor — system check\n");

    // vargc version
    println!("  [OK] vargc version  : {}", env!("CARGO_PKG_VERSION"));
    ...
    match Command::new("rustc").arg("--version").output() {
        ...
        _ => println!("  [!!] rustc          : not found — install Rust via https://rustup.rs"),
    }
    ...
    match Command::new("cargo").arg("--version").output() {
        ...
        _ => println!("  [!!] cargo          : not found — install Rust via https://rustup.rs"),
    }

    // VARG_LLM_PROVIDER
    match std::env::var("VARG_LLM_PROVIDER") {
        Ok(val) => println!("  [OK] VARG_LLM_PROVIDER : {}", val),
        Err(_)  => println!("  [OK] VARG_LLM_PROVIDER : (not set, defaults to ollama)"),
    }

    // OPENAI_API_KEY
    match std::env::var("OPENAI_API_KEY") {
        Ok(_)  => println!("  [OK] OPENAI_API_KEY    : set"),
        Err(_) => println!("  [!!] OPENAI_API_KEY    : not set"),
    }

    // ANTHROPIC_API_KEY
    match std::env::var("ANTHROPIC_API_KEY") {
        Ok(_)  => println!("  [OK] ANTHROPIC_API_KEY : set"),
        Err(_) => println!("  [!!] ANTHROPIC_API_KEY : not set"),
    }

    // Global cache / shared target dir
    let cache = dirs::cache_dir()
        .unwrap_or_default()
        .join("varg")
        .join("target");
    println!("  [OK] global cache   : {}", cache.display());

    println!();
    println!("Run 'vargc build hello.varg' to test the full pipeline.");
}

// ── Wave 42: upgrade ───────────────────────────────────────────────────────────

/// `vargc upgrade` — download and install the latest vargc release
fn cmd_upgrade() {
    const API_URL: &str =
        "https://api.github.com/repos/LupusMalusDeviant/VARG/releases/latest";

    println!("Fetching latest release...");

    // Use curl to fetch the GitHub API response (avoids adding reqwest/tokio to vargc)
    let curl_out = Command::new("curl")
        .args(["-sL", API_URL])
        .output();

    let body = match curl_out {
        Ok(out) if out.status.success() => {
        ...
        Err(e) => {
            ...
            eprintln!("Make sure curl is installed and on your PATH.");
            exit(1);
        }
    };
    ...
    let assets = match release["assets"].as_array() {
        Some(a) => a,
        None => {
            eprintln!("Error: no assets found in release {}.", tag);
            exit(1);
        }
    };

    let asset = assets.iter().find(|a| {
        a["name"].as_str().unwrap_or("").to_lowercase().contains(platform_key)
    });

    let asset = match asset {
        Some(a) => a,
        None => {
            eprintln!("Error: no {} asset found in release {}.", platform_key, tag);
            eprintln!("Available assets:");
            for a in assets {
                eprintln!("  {}", a["name"].as_str().unwrap_or("(unnamed)"));
            }
            exit(1);
        }
    };
...
// All vargc compilations share one Cargo target dir so dependencies are only
// compiled once.  After the first ~35 s warm-up subsequent builds take ~2-3 s.
fn global_target_dir() -> PathBuf {
    dirs::cache_dir()
...
const FEATURE_MAP: &[(&str, &str)] = &[
    ...
    ("varg_runtime::tensor::",     "tensor"),
    ("varg_runtime::dataframe::",  "dataframe"),
    ...
    ("__varg_fetch",               "net"),
    // Checksum-verified package download: HTTP fetch (net) + SHA-256 (crypto). Listed twice
...
fn run_cli() {
    ...
    // Wave 42: System diagnostics — no file argument needed
    if command == "doctor" {
        cmd_doctor();
        return;
    }
...
fn print_usage() {
    ...
    println!();
    println!("Toolchain:");
    println!("  vargc doctor              Print system diagnostics (Rust, env vars, cache dir)");
    println!("  vargc upgrade             Download and install the latest vargc release");
    println!();
...
/// Wave 47: Generate HTML API documentation from a .varg source file.
/// Writes `{stem}.html` next to the source file. Self-contained (no external deps).
fn generate_docs(input_file: &str) {
    ...
    let module_name = stem.as_ref();

    // ── Build content sections ────────────────────────────────────────────────
    let mut toc  = String::new();   // sidebar links
    let mut body = String::new();   // main content

    for item in &program.items {
        let (kind, name, kind_slug, color) = match item {
            varg_ast::ast::Item::Agent(a)    => ("Agent",    a.name.as_str(), "agent",    "#4ec9b0"),
            varg_ast::ast::Item::Contract(c) => ("Contract", c.name.as_str(), "contract", "#9cdcfe"),
            varg_ast::ast::Item::Struct(s)   => ("Struct",   s.name.as_str(), "struct",   "#dcdcaa"),
            varg_ast::ast::Item::Enum(e)     => ("Enum",     e.name.as_str(), "enum",     "#c586c0"),
            varg_ast::ast::Item::Function(f) => ("Function", f.name.as_str(), "fn",       "#569cd6"),
            _ => continue,
        };

        let anchor = format!("{}-{}", kind_slug, name);
        toc.push_str(&format!(
            "<a href=\"#{}\" class=\"toc-{}\"><span class=\"toc-badge\">{}</span>{}</a>\n",
            anchor, kind_slug, kind_slug, name
        ));

        body.push_str(&format!("<section id=\"{}\" class=\"def-block\">\n", anchor));
        body.push_str(&format!(
            "<h2><span class=\"kind\" style=\"color:{}\">{}</span> {}</h2>\n",
            color, kind, name
        ));

        if let Some(doc) = program.docs.get(name) {
            let escaped = html_escape(doc);
            body.push_str(&format!("<p class=\"doc\">{}</p>\n", escaped.replace('\n', "<br>")));
        }
        ...
        if let Some(methods) = methods {
            if !methods.is_empty() {
                body.push_str("<h3>Methods</h3><ul class=\"member-list\">\n");
                for m in methods {
                    let vis       = if m.is_public { "public " } else { "" };
                    let async_kw  = if m.is_async  { "async " }  else { "" };
                    let args: Vec<String> = m.args.iter()
                        .map(|a| format!("{} {}", format_type(&a.ty), a.name)).collect();
                    let ret = m.return_ty.as_ref()
                        .map(|t| format!(" -&gt; {}", html_escape(&format_type(t))))
                        .unwrap_or_default();
                    let sig = format!("{}{}{}{}{}", vis, async_kw, html_escape(m.name.as_str()),
                        if args.is_empty() { "()".to_string() }
                        else { format!("({})", html_escape(&args.join(", "))) },
                        ret);
        ...
        if let varg_ast::ast::Item::Struct(s) = item {
            if !s.fields.is_empty() {
                body.push_str("<h3>Fields</h3><ul class=\"member-list\">\n");
                for f in &s.fields {
                    body.push_str(&format!("<li><code>{}: {}</code></li>\n",
                        html_escape(&f.name), html_escape(&format_type(&f.ty))));
                }
                body.push_str("</ul>\n");
            }
        }
        ...
        if let varg_ast::ast::Item::Enum(e) = item {
            body.push_str("<h3>Variants</h3><ul class=\"member-list\">\n");
            for v in &e.variants {
                let sig = if v.fields.is_empty() {
                    html_escape(&v.name)
                } else {
                    let fields: Vec<String> = v.fields.iter()
                        .map(|(n, t)| format!("{}: {}", n, format_type(t))).collect();
                    html_escape(&format!("{}({})", v.name, fields.join(", ")))
                };
                body.push_str(&format!("<li><code>{}</code></li>\n", sig));
            }
            body.push_str("</ul>\n");
...
<title>{module} — Varg API Docs</title>
<style>
  :root {{ --bg:#0c0e22; --sidebar:#10122e; --accent:#4ec9b0; --text:#cdd6f4;
           --code-bg:#1a1e50; --border:#2a3070; --muted:#6a9955; }}
  * {{ box-sizing:border-box; margin:0; padding:0; }}
  ...
  #sidebar h1 {{ font-size:.9rem; color:var(--muted); text-transform:uppercase;
                 letter-spacing:.1em; margin-bottom:1rem; }}
  .toc-badge {{ display:inline-block; font-size:.65rem; padding:.1em .4em;
                border-radius:3px; background:var(--code-bg); margin-right:.4em;
                font-family:monospace; }}
  #sidebar a {{ display:block; padding:.35rem .5rem; color:var(--text); text-decoration:none;
                border-radius:4px; font-size:.9rem; margin-bottom:.2rem; }}
  #sidebar a:hover {{ background:var(--border); }}
  .toc-agent   .toc-badge {{ color:#4ec9b0; }}
  .toc-contract .toc-badge {{ color:#9cdcfe; }}
  .toc-struct  .toc-badge {{ color:#dcdcaa; }}
  .toc-enum    .toc-badge {{ color:#c586c0; }}
  .toc-fn      .toc-badge {{ color:#569cd6; }}
  #content {{ padding:2.5rem 3rem; max-width:900px; }}
  #content h1 {{ font-size:1.6rem; color:var(--accent); margin-bottom:.3rem; }}
  #content .subtitle {{ color:var(--muted); margin-bottom:2rem; font-size:.9rem; }}
  .def-block {{ border:1px solid var(--border); border-radius:8px; padding:1.5rem;
                margin-bottom:1.5rem; background:var(--sidebar); }}
  .def-block h2 {{ font-size:1.2rem; margin-bottom:.8rem; }}
  .kind {{ font-size:.8rem; font-weight:700; text-transform:uppercase;
           letter-spacing:.07em; margin-right:.5em; font-family:monospace; }}
  .doc {{ color:#a8b5d1; line-height:1.6; margin-bottom:1rem; font-size:.95rem; }}
  h3 {{ font-size:.9rem; color:var(--muted); text-transform:uppercase;
        letter-spacing:.08em; margin:.8rem 0 .4rem; }}
  .member-list {{ list-style:none; padding-left:0; }}
  .member-list li {{ margin:.3rem 0; }}
  code {{ font-family:'Cascadia Code','Fira Code',Consolas,monospace; font-size:.88rem;
          background:var(--code-bg); padding:.15em .4em; border-radius:3px; color:#dcdcaa; }}
...
<main id="content">
  <h1>{module}</h1>
  <p class="subtitle">Generated by <code>vargc doc</code> · Varg v1.0</p>
  {body}
  <div class="footer">Generated by vargc doc · <a href="https://github.com/LupusMalusDeviant/VARG" style="color:var(--muted)">Varg Compiler</a></div>
</main>
</body>
</html>
"#,
    ...
    println!("-> Docs written to {}", out_path.display());
}
...
{}{}{}"#, varg_name,
    ...
    // Wave 35: Share one global target dir so deps compile only once (~35s first
    // time, ~2-3s on all subsequent builds regardless of source file).
    let gtd = global_target_dir();
    if !is_wasm {
        cmd.env("CARGO_TARGET_DIR", &gtd);
    }
    ...
    if is_wasm {
        ...
        let profile_dir = if debug_mode { "debug" } else { "release" };
        // Wave 35: Use global target dir (shared across all builds) if not WASM.
        let target_base = if !is_wasm {
            global_target_dir()
        } else {
